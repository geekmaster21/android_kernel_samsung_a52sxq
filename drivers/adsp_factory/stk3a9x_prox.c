...
#define PROX_UB_CELL_ID_PATH "/efs/FactoryApp/prox_ub_cell_id"
...
int prox_load_ub_cell_id_from_file(char *path, char *data_str)
{
	...
	ret = vfs_read(file_filp, (char *)data_str,
		sizeof(char) * UB_CELL_ID_INFO_STRING_LENGTH,
		&file_filp->f_pos);
	data_str[UB_CELL_ID_INFO_STRING_LENGTH - 1] = '\0';
...
int prox_save_ub_cell_id_to_efs(char *path, char *data_str, bool first_booting)
{
	struct file *file_filp = NULL;
	mm_segment_t old_fs;
	int flag, ret = 0;
	...
	if (IS_ERR(file_filp)) {
		set_fs(old_fs);
		ret = PTR_ERR(file_filp);
		pr_err("[FACTORY] %s: open fail ub_cell_id(%s): %d\n",
			__func__, path, ret);
		return ret;
	}
...
void prox_cal_init_work(struct adsp_data *data)
{
	...
	cal_filp = filp_open(PROX_UB_CELL_ID_PATH, O_RDONLY, 0440);
	if (PTR_ERR(cal_filp) == -ENOENT || PTR_ERR(cal_filp) == -ENXIO) {
		pr_info("[FACTORY] %s : no prox_ub_efs file\n", __func__);
		temp_str = kzalloc(UB_CELL_ID_INFO_STRING_LENGTH,
			GFP_KERNEL);
		prox_save_ub_cell_id_to_efs(PROX_UB_CELL_ID_PATH, temp_str, true);
		kfree(temp_str);
	} else if (IS_ERR(cal_filp)) {
		pr_err("[FACTORY]: %s - ub filp_open error\n", __func__);
	} else {
		ret = vfs_read(cal_filp, (char *)data->prox_ub_id,
			sizeof(char) * UB_CELL_ID_INFO_STRING_LENGTH,
			&cal_filp->f_pos);
		data->prox_ub_id[UB_CELL_ID_INFO_STRING_LENGTH - 1] = '\0';
		if (ret < 0)
			pr_err("[SSC_FAC] %s: ub read fail:%d\n",
				__func__, ret);
		else
			pr_info("[SSC_FAC] %s : already ub efs exist %s\n",
				__func__, data->prox_ub_id);

		filp_close(cal_filp, current->files);
	}

	set_fs(old_fs);
	schedule_delayed_work(&data->prox_cal_work, msecs_to_jiffies(8000));
}

void prox_cal_read_work_func(struct work_struct *work)
{
	struct adsp_data *data = container_of((struct delayed_work *)work,
		struct adsp_data, prox_cal_work);
	char *cur_data_str = kzalloc(UB_CELL_ID_INFO_STRING_LENGTH,
				GFP_KERNEL);

	prox_load_ub_cell_id_from_file(UB_CELL_ID_PATH, cur_data_str);
	if (strcmp(data->prox_ub_id, cur_data_str) == 0) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (cnt >= TIMEOUT_CNT) {
		...
		if (cal_data > 0) {
			cur_id_str = kzalloc(UB_CELL_ID_INFO_STRING_LENGTH,
					GFP_KERNEL);
			...
			prox_load_ub_cell_id_from_file(UB_CELL_ID_PATH, cur_id_str);
			prox_save_ub_cell_id_to_efs(PROX_UB_CELL_ID_PATH, cur_id_str, false);
